...
!se CVMix_kpp, only : CVMix_kpp_compute_ER_Richardson
...
!wglevi  integer :: id_EnhW     = -1
  ...
  integer :: id_Lam2     = -1
!wglevi  integer :: id_BEdE = -1, id_ER_Ri    = -1
!wglevi  integer :: id_PTKE = -1
  integer :: id_PU_TKE = -1, id_PS_TKE = -1, id_PB_TKE = -1
  !>@}
...
!/ 2. Options related to enhancing the unresolved Vt2/entrainment in Rib
  ...
  CS%id_PU_TKE    = register_diag_field('ocean_model', 'PU_TKE' , diag%axesT1, Time, &
      'Shear fraction of Total SL TKE Production', 'm3 s-3')
  CS%id_PS_TKE    = register_diag_field('ocean_model', 'PS_TKE' , diag%axesT1, Time, &
      'Stokes fraction of Total SL TKE Production', 'm3 s-3')
  CS%id_PB_TKE    = register_diag_field('ocean_model', 'PB_TKE' , diag%axesT1, Time, &
      'Buoyancy fraction of Total SL TKE Production', 'm3 s-3')
  ...
  allocate( CS%ustar   ( SZI_(G), SZJ_(G) ), source=0. )
  ...
  if (CS%id_RNdepth > 0)  allocate( CS%RNdepth( SZI_(G), SZJ_(G) ), source=0. )
...
subroutine KPP_compute_BLD(CS, G, GV, US, h, Temp, Salt, u, v, tv, uStar, buoyFlux, Waves, lamult)
  ...
  real :: PU_TKE, PS_TKE, PB_TKE !wgl  Shear, Stokes, Buoyancy TKE production rate  [ m3 s-3 ]
  real, dimension( GV%ke )   :: StokesXI_1d !wglevi, StokesVt_1d  !  Parameters of TKE production ratio [nondim]
  real, dimension( GV%ke )   :: BEdE_ER_1d                ! Entrainment Rule parameterized  [Z^3 T-3 ~> m s-1]
  real :: ERdepth ! Entrainmnent Rule Boundary layer depth  CVMix_kpp_compute_ER_depth in MKS units [m]
  ...
  ! loop over horizontal points on processor                        !wglevi
  ...
  !$OMP                           StokesXI, StokesXI_1d, kbl, ERdepth, RNdepth, Vt_layer), &
  ...
  do j = G%jsc, G%jec
    do i = G%isc, G%iec ; if (G%mask2dT(i,j) > 0.0) then
      ...
      if (CS%StokesMOST) then
                        ! Load surface Stokes uS_Hi(1), vS_Hi(1)
        call Compute_StokesDrift(i, j, h(i,j,1), iFaceHeight(1),        & !wglevi h or dz ?
...
!wglevi                                                 use dz or h ?
      ...
      if ( (buoyFlux(i,j,3) .gt. 0.0) .and. (buoyFlux(i,j,3) .lt. buoyFlux(i,j,2)) ) then
      etaDk = alog(buoyFlux(i,j,2)/buoyFlux(i,j,3)) / dz(i,j,2)     ! (z_inter(2)-z_inter(3))
      ...
      iFaceHeight(1) = 0.0 !wglevi  BBL is always relative to the surface
      do k=1,GV%ke
        ! cell center and cell bottom in meters (negative values in the ocean)
        ...
        if (CS%StokesMOST) then
          ...
          uE_H(k) = U_H(k) - 0.5 * (Waves%US_x(I,j,k)+Waves%US_x(I-1,j,k))   !TEST_LEVY_9b
...
!wglevi   StokesVt_1d(k) = 0.0  ! StokesXI
...
!wglevi ******************************************************************************************************
      ...
      if (CS%id_ERdepth > 0) CS%ERdepth(i,j) = 0.0
      if (CS%id_RNdepth > 0) CS%RNdepth(i,j) = 0.0
...
!                Search for ER_depth
          ...
          if ( ERdepth > -z_inter(2) ) then
            ...
            CS%kOBL(i,j)     = CVMix_kpp_compute_kOBL_depth( iFaceHeight, cellHeight, CS%OBLdepth(i,j) )
            if (CS%id_ERdepth > 0) CS%ERdepth(i,j) = 100.
          endif
        endif

        if ( (.not.CS%StokesMOST) .or. (ERdepth < -z_inter(2)) ) then
!              Compute KPP_OBL_depth via Richardason Number
          ...
          if (CS%id_RNdepth > 0) CS%RNdepth(i,j) = 100.
        endif  !                  KPP_OBL_depth
      ...
      do k = 1, GV%ke
        BulkRi_1d(k) =  deltaBuoy(kbl) * CS%OBLdepth(i,j) / ( deltaU2(k) + Vt2_1d(k) )
      enddo
      if (CS%id_BulkRi > 0)  then
        CS%BulkRi(i,j,:) = BulkRi_1d(:)
      endif
  ...
  call pass_var(CS%PS_TKE(:,:), G%Domain, halo=1)
  call pass_var(CS%PU_TKE(:,:), G%Domain, halo=1)
  call pass_var(CS%PB_TKE(:,:), G%Domain, halo=1)
  ...
  if (CS%id_N2       > 0) call post_data(CS%id_N2,       CS%N2,              CS%diag)  !OK
  ...
  if (CS%id_BulkRi   > 0) call post_data(CS%id_BulkRi,  CS%BulkRi,        CS%diag)  !OK
...
!> Copies KPP TKE terms
subroutine KPP_get_TKE(CS, PS_TKE, PU_TKE, PB_TKE, G, US)
  type(KPP_CS),                     pointer     :: CS  !< Control structure for
                                                       !! this module
  type(ocean_grid_type),            intent(in)  :: G   !< Grid structure
  type(unit_scale_type),            intent(in)  :: US  !< A dimensional unit scaling type
  real, dimension(SZI_(G),SZJ_(G)), intent(inout) :: PS_TKE !< Stokes
  real, dimension(SZI_(G),SZJ_(G)), intent(inout) :: PU_TKE !< Shear
  real, dimension(SZI_(G),SZJ_(G)), intent(inout) :: PB_TKE !< Bouyancy
  ! Local variables
  integer :: i,j
  ...
  !$OMP parallel do default(none) shared(PS_TKE, PU_TKE, PB_TKE, CS, G)
  do j = G%jsd, G%jed ; do i = G%isd, G%ied
    PS_TKE(i,j) = CS%PS_TKE(i,j)
    PU_TKE(i,j) = CS%PU_TKE(i,j)
    PB_TKE(i,j) = CS%PB_TKE(i,j)
  enddo ; enddo

end subroutine KPP_get_TKE
