...
public register_IDEAL_IS_tracer, initialize_IDEAL_IS_tracer
...
type, public :: IDEAL_IS_tracer_CS ; private
  ...
  logical :: use_sponge
  logical :: shelf_thermo  ! if true, dye the melt water under ice shelves
...
end type IDEAL_IS_tracer_CS

contains


!> This subroutine is used to register tracer fields
function register_IDEAL_IS_tracer(HI, GV, param_file, CS, tr_Reg, &
                                      restart_CS)
  ...
  type(IDEAL_IS_tracer_CS),       pointer    :: CS !<A pointer that is set to point to the control structure for this module (in/out).
...
#include "version_variable.h"
  character(len=40)  :: mod = "IDEAL_IS_tracer" ! This module's name.
  character(len=200) :: inputdir
  real, pointer :: tr_ptr(:,:,:) => NULL()
  logical :: register_IDEAL_IS_tracer
  integer :: isd, ied, jsd, jed, nz, m
  isd = HI%isd ; ied = HI%ied ; jsd = HI%jsd ; jed = HI%jed ; nz = GV%ke

  if (associated(CS)) then
    call MOM_error(WARNING, "IDEAL_IS_register_tracer called with an "// &
  ...
  call get_param(param_file, mod, "IDEAL_IS_TRACER_IC_FILE", CS%tracer_IC_file, &
  ...
  if (len_trim(CS%tracer_IC_file) >= 1) then
    ...
    call log_param(param_file, mod, "INPUTDIR/IDEAL_IS_TRACER_IC_FILE", &
                   CS%tracer_IC_file)
  endif
  ...
  do m=1,NTR
    ...
    if (CS%coupled_tracers) &
      CS%ind_tr(m) = aof_set_coupler_flux(trim(name)//'_flux', &
          flux_type=' ', implementation=' ', caller="register_IDEAL_IS_tracer")
  enddo

  CS%tr_Reg => tr_Reg
  register_IDEAL_IS_tracer = .true.
  ...
  if (.not.restart) then
    if (len_trim(CS%tracer_IC_file) >= 1) then
      ...
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call MOM_error(FATAL, "IDEAL_IS_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
        call query_vardesc(CS%tr_desc(m), name, caller="initialize_IDEAL_IS_tracer")
...
end subroutine initialize_IDEAL_IS_tracer
...
end subroutine IDEAL_IS_tracer_column_physics

!> This particular tracer package does not report anything back to the coupler.
! The code that is here is just a rough guide for packages that would.
subroutine IDEAL_IS_tracer_surface_state(state, h, G, CS)
...
end subroutine IDEAL_IS_tracer_surface_state

subroutine IDEAL_IS_tracer_end(CS)
  type(IDEAL_IS_tracer_CS), pointer :: CS
  integer :: m
...
end subroutine IDEAL_IS_tracer_end

end module IDEAL_IS_tracer
