...
use OUT_IS_tracer, only : register_OUT_IS_tracer, initialize_OUT_IS_tracer
use OUT_IS_tracer, only : OUT_IS_tracer_column_physics, OUT_IS_tracer_surface_state
use OUT_IS_tracer, only : OUT_IS_tracer_end, OUT_IS_tracer_CS
...
type, public :: tracer_flow_control_CS ; private
  ...
  logical :: use_OUT_IS_tracer = .false.
  ...
  type(OUT_IS_tracer_CS), pointer :: OUT_IS_tracer_CSp => NULL()
...
#include "version_variable.h"
  ...
  call get_param(param_file, mod, "USE_ISOMIP_TRACER", CS%use_ISOMIP_tracer, &
                 ...
                 default=.false.)
  call get_param(param_file, mod, "USE_OUT_IS_TRACER", CS%use_OUT_IS_tracer, &
                 "If true, use the OUT_IS_tracer tracer package.", &
                 default=.false.)
...
!  for some reason.  This then overrides the run-time selection from above.
  ...
  if (CS%use_ISOMIP_tracer) CS%use_ISOMIP_tracer = &
    register_ISOMIP_tracer(HI, GV, param_file, CS%ISOMIP_tracer_CSp, &
                           tr_Reg, restart_CS)
  if (CS%use_OUT_IS_tracer) CS%use_OUT_IS_tracer = &
    register_OUT_IS_tracer(HI, GV, param_file, CS%OUT_IS_tracer_CSp, &
                           tr_Reg, restart_CS)
...
!  Add other user-provided calls here.
  ...
  if (CS%use_OUT_IS_tracer) &
    call initialize_OUT_IS_tracer(restart, day, G, GV, h, diag, OBC, CS%OUT_IS_tracer_CSp, &
                                sponge_CSp, diag_to_Z_CSp)
  if (CS%use_ideal_age) &
  ...
  if (present(evap_CFL_limit) .and. present(minimum_forcing_depth)) then
    ...
    if (CS%use_ISOMIP_tracer) &
      call ISOMIP_tracer_column_physics(h_old, h_new, ea, eb, fluxes, dt, &
                                        ...
                                        minimum_forcing_depth=minimum_forcing_depth)
    if (CS%use_OUT_IS_tracer) &
      call OUT_IS_tracer_column_physics(h_old, h_new, ea, eb, fluxes, dt, &
                                        G, GV, CS%OUT_IS_tracer_CSp, &
                                        evap_CFL_limit=evap_CFL_limit, &
                                        minimum_forcing_depth=minimum_forcing_depth)
    if (CS%use_ideal_age) &
...
!  Add other user-provided calls here.
  ...
  if (CS%use_OUT_IS_tracer) &
    call OUT_IS_tracer_surface_state(state, h, G, CS%OUT_IS_tracer_CSp)
  if (CS%use_ideal_age) &
...
subroutine tracer_flow_control_end(CS)
  ...
  if (CS%use_OUT_IS_tracer) call OUT_IS_tracer_end(CS%OUT_IS_tracer_CSp)
