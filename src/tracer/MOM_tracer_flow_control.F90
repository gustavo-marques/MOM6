...
use IDEAL_IS_tracer, only : register_IDEAL_IS_tracer, initialize_IDEAL_IS_tracer
use IDEAL_IS_tracer, only : IDEAL_IS_tracer_column_physics, IDEAL_IS_tracer_surface_state
use IDEAL_IS_tracer, only : IDEAL_IS_tracer_end, IDEAL_IS_tracer_CS
...
type, public :: tracer_flow_control_CS ; private
  ...
  logical :: use_IDEAL_IS_tracer = .false.
  ...
  type(IDEAL_IS_tracer_CS), pointer :: IDEAL_IS_tracer_CSp => NULL()
...
#include "version_variable.h"
  ...
  call get_param(param_file, mod, "USE_ISOMIP_TRACER", CS%use_ISOMIP_tracer, &
                 ...
                 default=.false.)
  call get_param(param_file, mod, "USE_IDEAL_IS_TRACER", CS%use_IDEAL_IS_tracer, &
                 "If true, use the IDEAL_IS_tracer tracer package.", &
                 default=.false.)
...
!  for some reason.  This then overrides the run-time selection from above.
  ...
  if (CS%use_ISOMIP_tracer) CS%use_ISOMIP_tracer = &
    register_ISOMIP_tracer(HI, GV, param_file, CS%ISOMIP_tracer_CSp, &
                           tr_Reg, restart_CS)
  if (CS%use_IDEAL_IS_tracer) CS%use_IDEAL_IS_tracer = &
    register_IDEAL_IS_tracer(HI, GV, param_file, CS%IDEAL_IS_tracer_CSp, &
                           tr_Reg, restart_CS)
...
!  Add other user-provided calls here.
  ...
  if (CS%use_IDEAL_IS_tracer) &
    call initialize_IDEAL_IS_tracer(restart, day, G, GV, h, diag, OBC, &
                  CS%IDEAL_IS_tracer_CSp,sponge_CSp, diag_to_Z_CSp)
...
!  Add other user-provided calls here.
  ...
  if (CS%use_IDEAL_IS_tracer) &
    call IDEAL_IS_tracer_surface_state(state, h, G, CS%IDEAL_IS_tracer_CSp)
  if (CS%use_ideal_age) &
...
subroutine tracer_flow_control_end(CS)
  ...
  if (CS%use_IDEAL_IS_tracer) call IDEAL_IS_tracer_end(CS%IDEAL_IS_tracer_CSp)
