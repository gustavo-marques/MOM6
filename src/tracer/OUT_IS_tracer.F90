...
!! (1) register_OUT_IS_tracer
!! (2) apply diffusion, physics/chemistry and advect the tracer 

!********+*********+*********+*********+*********+*********+*********+**
!*                                                                     *
!*  By Robert Hallberg, 2002                                           *
!*  Adapted to the IDEAL_IS test case by Gustavo Marques, Feb 2017     *            
!*********+*********+*********+*********+*********+*********+***********

module OUT_IS_tracer
...
public register_OUT_IS_tracer, initialize_OUT_IS_tracer
public OUT_IS_tracer_column_physics, OUT_IS_tracer_surface_state, OUT_IS_tracer_end
...
type, public :: OUT_IS_tracer_CS ; private
...
end type OUT_IS_tracer_CS

contains


!> This subroutine is used to register tracer fields
function register_OUT_IS_tracer(HI, GV, param_file, CS, tr_Reg, &
                                      restart_CS)
  ...
  type(OUT_IS_tracer_CS),       pointer    :: CS !<A pointer that is set to point to the control structure for this module (in/out).
...
#include "version_variable.h"
  character(len=40)  :: mod = "OUT_IS_tracer" ! This module's name.
  character(len=200) :: inputdir
  real, pointer :: tr_ptr(:,:,:) => NULL()
  logical :: register_OUT_IS_tracer
  integer :: isd, ied, jsd, jed, nz, m
  isd = HI%isd ; ied = HI%ied ; jsd = HI%jsd ; jed = HI%jed ; nz = GV%ke

  if (associated(CS)) then
    call MOM_error(WARNING, "OUT_IS_register_tracer called with an "// &
  ...
  call get_param(param_file, mod, "OUT_IS_TRACER_IC_FILE", CS%tracer_IC_file, &
                 "The name of a file from which to read the initial \n"//&
                 "conditions for the IDEAL_IS tracers, or blank to initialize \n"//&
                 "them internally.", default=" ")
  if (len_trim(CS%tracer_IC_file) >= 1) then
    call get_param(param_file, mod, "INPUTDIR", inputdir, default=".")
    inputdir = slasher(inputdir)
    CS%tracer_IC_file = trim(inputdir)//trim(CS%tracer_IC_file)
    call log_param(param_file, mod, "INPUTDIR/OUT_IS_TRACER_IC_FILE", &
                   CS%tracer_IC_file)
  endif
  ...
  do m=1,NTR
    ...
    write(longname,'("Concentration of IDEAL_IS Tracer ",I2.2)') m
    ...
    if (CS%coupled_tracers) &
      CS%ind_tr(m) = aof_set_coupler_flux(trim(name)//'_flux', &
          flux_type=' ', implementation=' ', caller="register_OUT_IS_tracer")
  enddo

  CS%tr_Reg => tr_Reg
  register_OUT_IS_tracer = .true.
end function register_OUT_IS_tracer

!> Initializes the NTR tracer fields in tr(:,:,:,:)
! and it sets up the tracer output. 
subroutine initialize_OUT_IS_tracer(restart, day, G, GV, h, diag, OBC, CS, &
  ...
  type(OUT_IS_tracer_CS),                pointer    :: CS !< The control structure returned by a previous call to OUT_IS_register_tracer.
  ...
  if (.not.restart) then
    if (len_trim(CS%tracer_IC_file) >= 1) then
      ...
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call MOM_error(FATAL, "OUT_IS_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
        call query_vardesc(CS%tr_desc(m), name, caller="initialize_OUT_IS_tracer")
  ...
  do m=1,NTR
    ...
    call query_vardesc(CS%tr_desc(m), name, units=units, longname=longname, &
                       caller="initialize_OUT_IS_tracer")
...
end subroutine initialize_OUT_IS_tracer

!> This subroutine applies diapycnal diffusion and any other column
! tracer physics or chemistry to the tracers from this file.
! This is a simple example of a set of advected passive tracers.
subroutine OUT_IS_tracer_column_physics(h_old, h_new,  ea,  eb, fluxes, dt, G, GV, CS, &
  ...
  real,                                  intent(in) :: dt
  type(OUT_IS_tracer_CS),                  pointer    :: CS
...
!  if ( CS%use_sponge ) then
    ...
    do j=js,je ; do i=is,ie
      ! add tracer
      if (G%geoLatT(i,j) <= (CS%lensponge)) CS%tr(i,j,:,m) = 1.0 ! all layers
      ! remove tracer
      if (G%geoLatT(i,j) > (CS%lenlat - CS%lensponge)) CS%tr(i,j,:,m) = 0.0
    enddo ; enddo
...
end subroutine OUT_IS_tracer_column_physics

!> This particular tracer package does not report anything back to the coupler.
! The code that is here is just a rough guide for packages that would.
subroutine OUT_IS_tracer_surface_state(state, h, G, CS)
...
end subroutine OUT_IS_tracer_surface_state

subroutine OUT_IS_tracer_end(CS)
  type(OUT_IS_tracer_CS), pointer :: CS
  integer :: m
...
end subroutine OUT_IS_tracer_end

end module OUT_IS_tracer
