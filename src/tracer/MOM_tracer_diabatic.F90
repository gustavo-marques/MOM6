...
subroutine tracer_vertdiff_Eulerian(h_old, ent, dt, tr, G, GV, &
  ...
  if (present(sink_rate)) then
    ...
    ! GMM, remove if (G%mask2dT(i,j) > 0.5) and check if this matters in terms of speed?
    ! We want to think (mask condition) about this carefully, keeping in mind other tracers and APIs.
    !$OMP do
    do j=js,je
      do i=is,ie ; if (G%mask2dT(i,j) > 0.5) then
        h_tr = h_old(i,j,1) + h_neglect
