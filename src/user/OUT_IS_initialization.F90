...
character(len=40) :: mod = "OUT_IS_initialization" ! This module's name.

! -----------------------------------------------------------------------------
! The following routines are visible to the outside world
! -----------------------------------------------------------------------------
public OUT_IS_initialize_sponges
public OUT_IS_initialize_thickness
public OUT_IS_initialize_temperature_salinity
! -----------------------------------------------------------------------------
...
subroutine OUT_IS_initialize_thickness ( h, G, GV, param_file, tv )
  ...
  case ( REGRIDDING_LAYER, REGRIDDING_RHO ) ! Initial thicknesses for isopycnal coordinates
    call get_param(param_file, mod, "OUT_IS_T_SUR",t_sur,'Temperature at the surface (interface)', default=-1.9)
    call get_param(param_file, mod, "OUT_IS_S_SUR", s_sur, 'Salinity at the surface (interface)',  default=33.8)
    call get_param(param_file, mod, "OUT_IS_T_BOT", t_bot, 'Temperature at the bottom (interface)', default=-1.9)
    call get_param(param_file, mod, "OUT_IS_S_BOT", s_bot,'Salinity at the bottom (interface)', default=34.55)
...
end subroutine OUT_IS_initialize_thickness

!> Initial values for temperature and salinity
subroutine OUT_IS_initialize_temperature_salinity ( T, S, h, G, GV, param_file, &
                                                    eqn_of_state)
  ...
  select case ( coordinateMode(verticalCoordinate) )
   ...
   case default
      call MOM_error(FATAL,"out_is_initialize: "// &
      "Unrecognized i.c. setup - set REGRIDDING_COORDINATE_MODE")
...
end subroutine OUT_IS_initialize_temperature_salinity

!> Sets up the the inverse restoration time (Idamp), and
! the values towards which the interface heights and an arbitrary
! number of tracers should be restored within each sponge.
subroutine OUT_IS_initialize_sponges(G, GV, tv, PF, use_ALE, CSp, ACSp)
  type(ocean_grid_type), intent(in) :: G    !< The ocean's grid structure.
  type(verticalGrid_type), intent(in) :: GV !< The ocean's vertical grid structure.
  type(thermo_var_ptrs), intent(in) :: tv   !< A structure containing pointers
                                            !! to any available thermodynamic
...
! Local variables
  ...
  character(len=40)  :: mod = "OUT_IS_initialize_sponges" ! This subroutine's name.
  ...
  call get_param(PF, mod, "OUT_IS_TNUDG", TNUDG, 'Nudging time scale for sponge layers (days)',  default=0.0)
...
!   Set up sponges for ISOMIP configuration
  call get_param(PF, mod, "MINIMUM_DEPTH", min_depth, &
   ...
   if (associated(CSp)) call MOM_error(FATAL, &
          "OUT_IS_initialize_sponges called with an associated control structure.")
   if (associated(ACSp)) call MOM_error(FATAL, &
          "OUT_IS_initialize_sponges called with an associated ALE-sponge control structure.")
  ...
  ! convert to 1 / seconds
   ...
   call get_param(PF, mod, "OUT_IS_SPONGE_FILE", state_file, &
   ...
   if (.not.file_exists(filename, G%Domain)) &
       call MOM_error(FATAL, " OUT_IS_initialize_sponges: Unable to open "//trim(filename))
...
end subroutine OUT_IS_initialize_sponges

!> \class OUT_IS_initialization
!!
!!  The module configures the ISOMIP test case.
end module OUT_IS_initialization
