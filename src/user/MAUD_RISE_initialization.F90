module MAUD_RISE_initialization
!***********************************************************************
...
character(len=40) :: mod = "MAUD_RISE_initialization" ! This module's name.

! -----------------------------------------------------------------------------
! The following routines are visible to the outside world
! -----------------------------------------------------------------------------
public MAUD_RISE_initialize_topography
public MAUD_RISE_initialize_thickness
public MAUD_RISE_initialize_temperature_salinity
public MAUD_RISE_initialize_sponges
...
subroutine MAUD_RISE_initialize_topography(D, G, param_file, max_depth)
...
#include "version_variable.h"
  character(len=40)  :: mod = "MAUD_RISE_initialize_topography" ! This subroutine's name.
  integer :: i, j, is, ie, js, je, isd, ied, jsd, jed
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed


  call MOM_mesg("  MAUD_RISE_initialization.F90, MAUD_RISE_initialize_topography: setting topography", 5)
...
end subroutine MAUD_RISE_initialize_topography
! -----------------------------------------------------------------------------

!> Initialization of thicknesses
subroutine MAUD_RISE_initialize_thickness ( h, G, GV, param_file, tv )
  ...
  case ( REGRIDDING_LAYER, REGRIDDING_RHO ) ! Initial thicknesses for isopycnal coordinates
    call get_param(param_file, mod, "MAUD_RISE_T_SUR",t_sur,'Temperature at the surface (interface)', default=6.0)
    call get_param(param_file, mod, "MAUD_RISE_S_SUR", s_sur, 'Salinity at the surface (interface)',  default=0.0)
    call get_param(param_file, mod, "MAUD_RISE_T_BOT", t_bot, 'Temperature at the bottom (interface)', default=0.0)
    call get_param(param_file, mod, "MAUD_RISE_S_BOT", s_bot,'Salinity at the bottom (interface)', default=0.0)
...
end subroutine MAUD_RISE_initialize_thickness

!> Initial values for temperature and salinity
subroutine MAUD_RISE_initialize_temperature_salinity ( T, S, h, G, GV, param_file, &
                                                    eqn_of_state)
  ...
  call get_param(param_file,mod,"REGRIDDING_COORDINATE_MODE", verticalCoordinate, &
            default=DEFAULT_COORDINATE_MODE)
  call get_param(param_file, mod, "MAUD_RISE_T_SUR",t_sur,'Temperature at the surface (interface)', default=-1.9)
  call get_param(param_file, mod, "MAUD_RISE_S_SUR", s_sur, 'Salinity at the surface (interface)',  default=33.8)
  call get_param(param_file, mod, "MAUD_RISE_T_BOT", t_bot, 'Temperature at the bottom (interface)', default=-1.9)
  call get_param(param_file, mod, "MAUD_RISE_S_BOT", s_bot,'Salinity at the bottom (interface)', default=34.55)
...
end subroutine MAUD_RISE_initialize_temperature_salinity

!> Sets up the the inverse restoration time (Idamp), and
! the values towards which the interface heights and an arbitrary
! number of tracers should be restored within each sponge.
subroutine MAUD_RISE_initialize_sponges(G, GV, tv, u, v, PF, use_ALE, CSp, ACSp)
  type(ocean_grid_type), intent(in) :: G    !< The ocean's grid structure.
  type(verticalGrid_type), intent(in) :: GV !< The ocean's vertical grid structure.
  type(thermo_var_ptrs), intent(in) :: tv   !< A structure containing pointers
                                            !! to any available thermodynamic
...
! Local variables
  ...
  character(len=40)  :: mod = "MAUD_RISE_initialize_sponges" ! This subroutine's name.
  ...
  call get_param(PF, mod, "MAUD_RISE_TNUDG", TNUDG, 'Nudging time scale for sponge layers (days)',  default=0.0)
  ...
  call get_param(PF, mod, "MINIMUM_DEPTH", min_depth, &
   ...
   if (associated(CSp)) call MOM_error(FATAL, &
          "MAUD_RISE_initialize_sponges called with an associated control structure.")
   if (associated(ACSp)) call MOM_error(FATAL, &
          "MAUD_RISE_initialize_sponges called with an associated ALE-sponge control structure.")
  ...
  ! convert to 1 / seconds
   ...
   call get_param(PF, mod, "MAUD_RISE_SPONGE_FILE", state_file, &
   ...
   if (.not.file_exists(filename, G%Domain)) &
       call MOM_error(FATAL, " MAUD_RISE_initialize_sponges: Unable to open "//trim(filename))
...
end subroutine MAUD_RISE_initialize_sponges

!> \class MAUD_RISE_initialization
!!
!!  The module configures the ISOMIP test case.
end module MAUD_RISE_initialization
